...
from cinturon_asteroides import Asteroid
...
# 13. Rastro Planetas
# 14. Música relajante
# 15. Cinturon de asyeroides entre marte y jupiter
# 16. Ajustar escala con la ruedecilla del ratón
...
rango_asteroides = (Marte_d + 50, Marte_d + ((Jupiter_d - Marte_d) / 2))
color_asteroide = (128, 128, 128)
cantidad_asteroides = 1500
rango_tamaños = (1, 10)
lista_asteroides = []
for i in range(cantidad_asteroides):
    asteroide = 0
    velocidad = random.randint(1, 6) * 0.01
    tamaño_radio = random.randint(rango_tamaños[0], rango_tamaños[1])
    asteroide = Asteroid(tamaño_radio, rango_asteroides, random.randint(0,360), color_asteroide, escala, radio_sol, velocidad)
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            # Ajustando escala con la tueda del ratón
            if event.button == 4:
                print('Rueda arriba')
                if escala > 1:
                    escala = escala - 1
    ...
    # ASTEROIDES
    for i in lista_asteroides:
        i.angulo = i.angulo + i.velocidad
        i.mostrar(pantalla, anchura_pantalla, altura_pantalla, escala)
